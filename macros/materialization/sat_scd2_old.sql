{%- macro sat_scd2_old(src_pk, src_hashdiff, src_payload, src_eff, src_ldts, src_source, source_model) -%}
...
records_to_insert AS (
    SELECT DISTINCT {{ dbtvault.alias_all(source_cols, 'stage') }}, 'INSERT' as TYPE
...
records_to_update AS (
        SELECT DISTINCT {{ dbtvault.alias_all(source_cols, 'latest_records') }}, 'UPDATE' as TYPE
        FROM latest_records
        LEFT JOIN source_data AS stage
        ON {{ dbtvault.multikey(src_pk, prefix=['latest_records','stage'], condition='=') }}
            WHERE {{ dbtvault.prefix([src_hashdiff], 'latest_records', alias_target='target') }} != {{ dbtvault.prefix([src_hashdiff], 'stage') }}
)
